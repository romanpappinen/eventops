...
import { runEventProcessor } from './event-processor.js';
...
logger.info(
    {
        ...
        eventProcessingBatchSize: env.EVENT_PROCESSING_BATCH_SIZE,
    },
    'Worker started'
);

async function main() {
    await Promise.all([runInvitationEmailWorker(), runInvitationCleanupSweep(), runEventProcessor()]);
}
